...
#################################### PLOT RESULTS MULTILEVEL #################################### 
MLData = np.loadtxt("results/MULTILEVEL/example_NONConst_IC_g_02/convergenge_multilevel_NONconstIC_g_02.csv", delimiter=",")
xx = MLData[:,0]
yy = MLData[:,1]
plt.loglog(xx, yy, '.-')
plt.loglog(xx, np.power(xx, -1/4), 'k-')
plt.loglog(xx, np.power(xx, -1/3), 'k-')
plt.show()