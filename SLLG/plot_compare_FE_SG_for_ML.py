...
### compute sparse grid cardianlities as in [Teckentrup] for several number of lev
...
# plot results ML
MLData = np.loadtxt("results/MULTILEVEL/g_02/convergenge_multilevel_NONconstIC_g_02.csv", delimiter=",")
plt.loglog(MLData[:,0], MLData[:,1], '.-')
plt.loglog(MLData[:,0], np.power(MLData[:,0],-1/3), '.-')
plt.show()
