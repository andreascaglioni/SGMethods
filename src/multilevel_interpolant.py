class MLInterpolant:
    """Class for multi level sparse grid interpolant like in [Teckentrup, Jantsch, Webster, Gunzburger (2015)].
    """    
    def __init__(self, SLIL):
        """Store Single level interpolants list (that behave li SGInterpolant class)

        Args:
            SLIL (list of SGInterpolant): List of sparse grid interpolants with appropriate multi-index sets
        """        
    ...
    def sample(self, FApprox):
        ...
        Args:
            FApprox (function): Function with 2 inputs: a parameter (1D double array of ANY length), and a level k (int >=0) that gives approximation accuracy level

        Returns:
            list of 2D arrays double: List of length self.nLevels. Each list element is a 2D array; each row is a sample from FApprox for a parameter in the corresponding sparse grid; 
        """        
    ...
    def interpolate(self, yy, MLSamplesF):
        """USe the interpolation oeprator to appeoximate the function F of which ML samples are given

        Args:
            yy (2D array double): each row is a parameter and can have any length; number of rows also arbitrary
            MLSamplesF (_type_): _description_

        Returns:
            2D array double: each row is the approximation of the function F in a parameter yy[i,:]
        """        
    ...
    def getMLTerms(self, yy, MLSamplesF):
        """Get terms of multi-level expansion split based on finite element spaces

        Args:
            yy (2D array double): each row is a parameter vector toe valuate
            MLSamplesF (list of 2D arrays double): k-th entry is 2D array with shape nY x kth physical space size representing (I_{K-k}}-I_{K-k-1}[u_k])

        Returns:
            list of 2D arrays: k-th term is 2D array; each row corresponds to a parameter in yy; each colum gives the finite element corrdinates
        """        
    ...
    def totalCost(self, costKK):
        ...
        Args:
            costKK (1D array of double): kth entry is the cost of computing 1 FE sample at level k
