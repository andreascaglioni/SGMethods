...
class midSet():
    ...
    def update(self, idx_margin):
        ...
        # sanity check for dimensinality
        assert(mid[-1] >= 0 and mid[-1] <= 1)
        # check if in reduced margin, 
        ...
        # self.midSet = np.sort(self.midSet, axis=1)
        ...
        # self.margin = np.sort(self.margin, axis=1)
        
        

# def optimalProfitMidSet(n, N, P, Iold=None):
#     """ Create a class midSet instance containing the n mids in N^N with maximum profit 
#         P(\nu)
#         Start with I_1 = {0} and grow it 1 mid at a time on margin"""
#     I = midSet(N)
#     alpha=2
#     beta=0.5
#     rho = 2**(beta*np.ceil(np.log(np.linspace(1,N,N))))
#     ProfitFun = lambda x : 2**((2*alpha-1)*np.sum(x))# - alpha * np.sum(np.log2(rho,2), where=(x>0)))
#     while I.numMids() < n:
#         # find profit maximizer NBB must be in reduced margin!
