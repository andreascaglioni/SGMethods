...
class SGInterpolant:
    ...
    def __init__(
        ...
        self.combination_coeffs = []  # Inclusion-exclusion coefficients. list(int)

        # list[np.ndarray[float]] of active multi-indices, i.e. for which it is necessary to evaluate a TP interpolant.
        # They are the multi-indices with non-zero combination_coeff. *and more than one node*
        # NB The [0,0, ...] is *excluded*
        # TODO change to only INDICES of active mids?
        self.active_mids = []
    ...
    def setup_interpolant(self):
        ...
        for n in range(self.card_mid_set):
            ...
            combin_coeff = 1
            # index of the 1st mid with 1st components = currentMid[0]+2 (don't
            ...
            #  in particular recall that 0 mid is never active
            if combin_coeff != 0 and np.linalg.norm(current_mid) > 0:
                ...
                active_dims = np.asarray(num_nodes_arr > 1).nonzero()[0]
                ...
                # NB Here self.map_tp_to_SG is List[np.ndarray[int]],
                # map_tp_to_SG[i] has shape (# nodes d0, # nodes d1, ..., # nodes dN)
                # Storing interpolated values in a matrix of this shape is standard for all TP interpolants
    ...
    def setup_SG(self):
        ...
        for idx_active, curr_active_tp_nodes in enumerate(self.active_tp_nodes_list):
            ...
            where_tp_in_sg.append(self.map_tp_to_SG[idx_active].reshape((-1, 1)))

        self.SG = SG
        self.num_nodes = SG.shape[0]
        self.where_tp_in_sg = where_tp_in_sg
