...
def compute_quadrature_wights_incl_excl(dim_samples, eps, sg_interp):
    ...
    # Change shape combi_coeffs to (1, -1) for breoadcasting later
    ...
    ww = np.sum(combi_coeffs * W_tp, axis=1)  # bradcast combi_coeffs

    return ww

def compute_1d_quadrature_weights(max_n, lev2knots, knots, interpolant="pw_lin_extrapolated"):
    """ Compute 1D quadrature weights associated to an interpolation rule, up to a max number of quadrature nodes.

    Args:
        max_n (int): The maximum index (exclusive) for which to compute quadrature weights.
        lev2knots (callable[int, [int]]): Maps an index to the number of knots.
        knots (callable[int, [np.ndarray[float]]]): The 1D knots, given the desired number from `lev2knots`.
        interpolant (str, optional): The 1D interpolation. Defaults to / curretnly only supports "pw_lin_extrapolated" (piecewise linear interpolant with extrapolation).

    Returns:
        np.ndarray[np.ndarray[float]]: 1D array of objects. Each entry corresponds to an index up to `max_n`. The j-th entry is an array of 1D quadrature weights corresponding to the 1D nodes for the j-th index.
    """
    
    assert interpolant == "pw_lin_extrapolated", "Currently only pw_lin_extrapolated interpolant supported."

    prim_integrand = lambda a, b, x : -a/sqrt(2*pi)*exp**(-x**2/2) + sqrt(pi/2)*b*erf(x/sqrt(2))

    ww_1d = np.empty(max_n, dtype=object)
    for n in range(max_n):  # index
        num_nodes = lev2knots(n)
        nodes = knots(num_nodes)
        ww_1d[n] = np.zeros(num_nodes)
        if num_nodes == 0:
            ww_1d[0] = np.array([1.0])
        else:
            # Node 0
            y, yn = nodes[0], nodes[1]
            f = lambda x : prim_integrand(1/(y-yn), -yn/(y-yn), x)
            int_y_yn = f[yn]-f[y]
            ww_1d[n][0] = norm.cdf(nodes[0]) + int_y_yn
