...
# TODO update __init__ docstring
...
class TPPwLinearInterpolator:
    ...
    def __init__(self, nodes_tuple, f_on_nodes):
        ...
        self.inteprolant = RegularGridInterpolator(
            self.nodes_tuple,
            self.f_on_nodes,
            method="linear",
            bounds_error=False,
        ...
        self.w = None  # quadrature weights
    ...
    def __call__(self, x_new):
        ...
        """
        return self.inteprolant(x_new)

    def compute_quadrature_weights(self, measure="gauss"):
        self.w = None
...
class TPPwQuadraticInterpolator:
    ...
    def __call__(self, x_new):
        ...
        ff = np.zeros(tuple(3 * np.ones(self.n_dims, dtype=int)) + (num_x, self.dim_f))
...
class TPPwCubicInterpolator:
    ...
    def __call__(self, x_new):
        ...
        # 3 format interpolation data
        ff = np.zeros(tuple(4 * np.ones(self.n_dims, dtype=int)) + (num_x, self.dim_f))
        it = np.nditer(ff[..., 0, 0], flags=["multi_index"])
        for i in it:
            vec_it = np.asarray(it.multi_index, dtype=int)
            vec_it = vec_it.reshape(-1, 1)
