...
def opt_guass_nodes_nest(n, p=2):
    """Use :py:func:`optimal_gaussian_nodes` to generate nodes after checking 
    that the number ``n`` indeed gives *nested* nodes.
    The nodes are Gauss-distrbuted on :math:`\mathbb{R}`.
    They can be used to define an optimal degree ``p+1`` piecewise poynomial
    interpolation in the :math:`L^2_{\mu}(\mathbb{R})` norm, where 
    :math:`\mu` is the Gaussian density. 
    Args:
        n (int): number of nodes s.t. :math:`n = 2^{i+1}-1` for some 
            :math:`i\in\mathbb{N}_0`.
        p (int, optional): Piecewise polynomial interpolant degree + 1 (i.e. the
            number of nodes needed to determine a piecewise polynomial of degree
