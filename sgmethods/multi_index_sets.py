...
def compute_mid_set_fast(P, minimum_profit, N):
    ...
    Args:
        ...
        N (int): Number of dimensions, i.e. the largest dimension that presents
            a non-zero index. E.g. [[1, 0], [0, 1]] gives N=2

    Returns:
        numpy.ndarray[int]: 2D array. Multi-index set (each row is a multi-index
        of support size ``N``).
    """
    ...
    if N == 0:
        ...
        nu_val = 0
        assert P(np.array([[nu_val]])) > 0  # sanity check profit for while bwlow to conclude
        while P(np.array([[nu_val]]) + 1) > minimum_profit:
            nu_val = nu_val + 1
        # at this point, nu is the largest mid with P > Pmin
        return np.linspace(0, nu_val, nu_val + 1, dtype=int).reshape((-1, 1))
    else:  # N>1
        mid_set = np.zeros((0, N), dtype=int)

        # check how large the LAST component can be
        val_nu_last = 0
        nu_n_extra = unitVector(N, N - 1)  # 1 in last component, 0 elsewhere
        nu_n_extra = np.reshape(nu_n_extra, (1, -1))  # must be 2D for profit
        
        while P(nu_n_extra) > minimum_profit:
            nu_n_extra[0, -1] += 1
        max_last_comp = nu_n_extra[0, -1] - 1
        ...
        # look only at the first N-1 components
        for last_comp_nu in range(0, max_last_comp + 1):
            nu_curr = np.zeros(N, dtype=int)
            nu_curr = last_comp_nu * unitVector(N, N - 1)
            nu_curr = np.reshape(nu_curr, [1, -1])
            profit_rec = minimum_profit / P(nu_curr)
            new_comps = compute_mid_set_fast(P, profit_rec, N - 1)
            new_mids = np.hstack(
                (
                    new_comps,
                    np.full((new_comps.shape[0], 1), last_comp_nu),
                )
            )
            mid_set = np.vstack((mid_set, new_mids))
